"""Train a vanilla (Elman) RNN on the UCI Ozone Level Detection dataset.

    python models/rnn/example.py --device auto --epochs 30

Same dataset/task as models/ltc/example.py, so the two are directly
comparable -- this is the "no time-awareness at all" baseline for that
comparison. Ozone rows have no time axis by default, so we treat each of the
72 hourly features as one sequence step -> (B, 72, 1).
"""
...
from model import RNNModel  # noqa: E402
...
def main() -> None:
    ...
    model = RNNModel(input_size=1, hidden_size=args.hidden_size, output_size=1).to(device)
