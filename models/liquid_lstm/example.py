"""Train Liquid-LSTM on the UCI Ozone Level Detection dataset.

    python models/liquid_lstm/example.py --device auto --epochs 30

Same task as models/ltc/example.py -- direct comparison. See model.py for
why this is a hybrid (LSTM gating + LTC-style continuous time), not a
reimplementation of a single paper.
"""
...
from model import LiquidLSTMModel  # noqa: E402
...
def main() -> None:
    ...
    model = LiquidLSTMModel(input_size=1, hidden_size=args.hidden_size, output_size=1).to(device)
