"""Train an LTC network on the UCI Ozone Level Detection dataset.

    python models/ltc/example.py --device auto --epochs 30

This is the same dataset family the original LTC paper benchmarks against
(see papers/README.md). Ozone rows have no time axis by default, so we treat
each of the 72 hourly features as one sequence step -> (B, 72, 1).
"""
...
from model import LTCModel  # noqa: E402
...
def main() -> None:
    ...
    model = LTCModel(input_size=1, hidden_size=args.hidden_size, output_size=1).to(device)
